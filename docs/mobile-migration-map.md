...
| Scanner UI | `src/app/scanner/page.tsx`, `src/components/scanner/ModelStatusBanner.tsx` | Flutter scanner screen preserving current visual hierarchy: camera surface, start/stop, camera chooser, runtime health, latest alert, seen plates, developer/dataset controls where permission-gated. | Scaffolded around native bridge with real detector/tracker/OCR events, runtime health, consensus, local matching, alerts, evidence paths, persisted selected camera, and history writes. Detector/OCR fallback events are disabled; real-device validation pending. |
...
| ONNX runtime setup | `src/lib/anpr/onnxRuntime.ts`, `public/ort-wasm/*` | Android `onnxruntime-android`; iOS `onnxruntime-objc`/mobile package. Hardware acceleration must be capability-gated with CPU fallback. | Android `onnxruntime-android` dependency and iOS `onnxruntime-objc` CocoaPods integration are implemented with release/signing safeguards, staged model session loaders, input/output metadata capture, fallback-safe provider status, and live detector/OCR session inference. |
| PP-OCR | `src/lib/anpr/ppOcrEngine.ts`, `src/lib/anpr/ocrEngine.ts`, `public/models/ppocr-rec.onnx`, `public/models/ppocr-dict.txt` | Native PP-OCR recognizer preserving CTC decoding, dictionary, two-line handling, candidate generation, confidence, and preprocessing variants. | Android/iOS typed OCR event path implemented with staged model/dictionary assets and shared Dart dictionary/greedy CTC decoder tests. Android and iOS now preprocess best plate crops into PaddleOCR `1x3x48x320` tensors, run staged PP-OCR ONNX sessions, decode CTC output, rank enhanced/inner/two-line/recovery/deskew candidates, and emit `CPU_ONNX_PP_OCR` only from real OCR output. Real-device OCR validation remains. |
| Tracker | `src/lib/anpr/tracker.ts`, `src/__tests__/tracker.fat.test.ts` | Native Kotlin/Swift equivalent of ByteTrack-inspired lifecycle: visible/lost/removed, short prediction, no ghost boxes, independent tracks. | Android/iOS native track lifecycle implemented for real detector events. Existing TS remains reference for clip-level parity. |
...
| 7 | Integrate native detector. | Android CameraX and iOS AVFoundation live YOLO ONNX multi-box tracker fanout implemented with heuristic fallback; real-device parity validation remains. |
| 8 | Integrate tracker. | Functional native tracker implemented for real detector events. |
...
| 11 | Integrate OCR. | Android and iOS PP-OCR ONNX tensor inference/CTC decoding/candidate ranking implemented with fallback safety; real-device OCR validation remains. |
